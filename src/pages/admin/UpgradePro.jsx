...
export default function UpgradePro() {
  ...
  const monthlyPrice = Number(pro.price) || 0
  const seatPrice = Number(pro.seatPrice) || 0
  const billedYearly = (pro.billingCycle || 'yearly') === 'yearly'
  const [support, setSupport] = useState({})
  useEffect(() => { api.get('/support').then(setSupport).catch(() => setSupport({})) }, [])
  ...
  const seatMonthly = totalSeats * seatPrice
  ...
  const seatMsg = encodeURIComponent(
    `Hi Wandra! ${agency.name} would like ${totalSeats} user${totalSeats === 1 ? '' : 's'} (${seatBreakdown || 'to discuss seats'}) at ₹${seatPrice.toLocaleString('en-IN')}/user/month — about ₹${seatMonthly.toLocaleString('en-IN')}/month. Please set these up for us.`,
  )
  const sendSeatRequest = (channel) => {
    if (!totalSeats) return toast('Add at least one user to request')
    const url = channel === 'wa'
      ? `https://wa.me/${String(support.whatsapp || support.phone || '').replace(/\D/g, '')}?text=${seatMsg}`
      : `mailto:${support.email || ''}?subject=${encodeURIComponent(`User seats request — ${agency.name}`)}&body=${seatMsg}`
                ...
                {i === 0 && (
                  <div className="up-contact">
                    <a className="up-contact-btn" target="_blank" rel="noreferrer"
                      href={`https://wa.me/${String(support.whatsapp || support.phone || '').replace(/\D/g, '')}?text=${waMsg}`}>
                      WhatsApp {support.whatsapp || support.phone || 'not configured'}
                    </a>
                    <a className="up-contact-btn" href={`mailto:${support.email || ''}?subject=${encodeURIComponent(`Pro upgrade payment proof — ${agency.name}`)}`}>
                      {support.email || 'Email not configured'}
                    </a>
                  </div>
                )}
              </div>
        ...
        <div className="up-side">
          ...
          <div className="up-card">
            ...
            <div className="up-seat-what">
              ...
              <span>A <strong>user</strong> is a login for one team member — their own email, password and role. Each user is <strong>{inr(seatPrice)}/user/month</strong>, billed to your agency. The owner account counts as one user.</span>
            </div>
          ...
          <div className="up-card">
            ...
            {[
              ['Account name', support.paymentAccountName],
              ['Bank', support.paymentBankName],
              ['Branch', support.paymentBranch],
              ['Account type', support.paymentAccountType],
              ['Account number', support.paymentAccountNumber],
              ['IFSC', support.paymentIfsc],
            ].map(([k, v]) => (
              <button className="up-pay-row" key={k} title="Copy" onClick={() => copy(v, k)}>
                <span className="up-pay-k">{k}</span>
                <span className="up-pay-v">{v || 'Not configured'}</span>
                <Icon name="copy" size={12} />
              </button>
            ))}
            {support.paymentQr && <div className="up-qr">
              <img src={support.paymentQr} alt="Scan to pay — UPI QR code" loading="lazy" />
