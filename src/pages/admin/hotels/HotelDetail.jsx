...
import { PageHeader, Card, Button, Badge, Modal, Field, Input, PillSelect, Textarea, CityPicker } from '../../../components/ui/UI'
...
export default function HotelDetail() {
  const { id } = useParams()
  const { hotels, packages, destinations, cities, updateHotel, toast } = useApp()
  ...
  const save = () => {
    updateHotel(h.id, {
      ...f, rating: Number(f.rating), buyingPrice: Number(f.buyingPrice),
      extraBedAdult: Number(f.extraBedAdult) || 0, extraBedChild: Number(f.extraBedChild) || 0,
      childNoBed: Number(f.childNoBed) || 0, infantCharge: Number(f.infantCharge) || 0,
      rates: cleanSeasonRates(f.rates, HOTEL_SEASON_FIELDS),
    })
    toast('Hotel updated'); setEdit(false)
  }
  const seasons = h.rates || []
    ...
    <div>
      ...
      <div className="detail-grid">
        ...
        <Card className="fin-card">
          ...
          {seasons.length > 0 && (<>
            <hr className="divider" />
            <span className="t-title-sm">Season-wise rates</span>
            {seasons.map((r, i) => (
              <div className="fin-line" key={r.id || i}>
                <span className="c-body">{r.label || 'Season'}<span className="cell-sub"> · {r.from || '—'} → {r.to || '—'}</span></span>
                <span className="cell-strong">{inr(r.buyingPrice ?? h.buyingPrice)}/night</span>
              </div>
            ))}
          </>)}
          <hr className="divider" />
      ...
      <Modal open={edit} onClose={() => setEdit(false)} title="Edit Hotel" width={820}
        ...
        <div className="form-grid">
          ...
          <Field label="City"><CityPicker value={f.city || ''} cities={cities} destination={f.destination} onChange={(v) => setF({ ...f, city: v })} /></Field>
          ...
          <Field label="Infant (₹ / night)" hint="usually 0"><Input value={f.infantCharge ?? ''} onChange={(e) => setF({ ...f, infantCharge: e.target.value })} /></Field>
